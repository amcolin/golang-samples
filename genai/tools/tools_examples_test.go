...
func TestTextGeneration(t *testing.T) {
	...
	t.Run("generate with func config declaration", func(t *testing.T) {
		buf.Reset()
		err := generateWithFuncCallConfig(buf)
		if err != nil {
			t.Fatalf("generateWithFuncCallConfig failed: %v", err)
		}
	...
	t.Run("generate content with parallel function", func(t *testing.T) {
		buf.Reset()
		err := generateWithParallelFunctionCalling(buf)
		if err != nil {
			t.Fatalf("generateWithParallelFunctionCalling failed: %v", err)
		}
